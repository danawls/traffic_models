...
def train_svr_model(data):
    ...
    # SVR 모델 학습
    svr_model = SVR(kernel='rbf')  # RBF 커널 사용
...
# SVR 모델 학습 및 예측
y_test, y_pred = train_svr_model(data)
...
plt.title('SVR Model: Actual vs Predicted Traffic(Q)')
plt.legend()
plt.grid()
plt.show()

# df = pd.DataFrame({'value': list(y_pred), 'real': list(y_test)})
# df.to_csv('/Users/danawls/Desktop/*Important*/traffic-deep-learning-research/table-figure/table/deep-compare/svm.csv', index=False)