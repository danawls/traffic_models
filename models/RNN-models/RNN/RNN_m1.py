...
file_list = sorted(glob.glob('/mnt/data/*.csv'))  # '1.csv', '2.csv', '3.csv' 등
seq_length = 10

# 모델 초기화 변수
model = None
pickle_path = 'vanilla_rnn_model.pkl'
...
for i, file_path in enumerate(file_list):
    ...
    if i == 0:
        # 초기 모델 구성 (첫 번째 파일만)
        input_rnn = Input(shape=(X_train.shape[1], X_train.shape[2]))

        # RNN Layer
        rnn_out = SimpleRNN(64, return_sequences=False)(input_rnn)

        # Dense Layer
        dense_out = Dense(32, activation='relu')(rnn_out)
        rnn_output = Dense(1)(dense_out)

        model = Model(inputs=input_rnn, outputs=rnn_output)
    ...
    plt.title(f'Vanilla RNN Model Predictions vs Actual (File: {file_path})')
    plt.xlabel('Date')
    plt.ylabel('Flow')
    plt.legend()
    plt.show()
