...
# 모델 초기화 변수
model = None
pickle_path = 'vanilla_lstm_model.pkl'
...
for i, file_path in enumerate(file_list):
    ...
    if i == 0:
        ...
        # LSTM Layer
        lstm_out = LSTM(64, return_sequences=False)(input_lstm)

        # Dense Layer
        dense_out = Dense(32, activation='relu')(lstm_out)
        lstm_output = Dense(1)(dense_out)

        model = Model(inputs=input_lstm, outputs=lstm_output)
    ...
    plt.title(f'LSTM Model Predictions vs Actual (File: {file_path})')
    plt.xlabel('Date')
    plt.ylabel('Flow')
    plt.legend()
    plt.show()
