...
def preprocess_data(file_path):
    ...
    # 변형이나 정규화 없이 원본 데이터 사용
    return data


# Auto ARIMA 모델 학습 및 예측 함수
...
if file_paths:  # 파일이 존재하는지 확인
    ...
    # Auto ARIMA 모델 학습 및 예측
    y_test, y_pred = train_auto_arima_model(data)
    ...
    plt.title('Auto ARIMA Model: Actual vs Predicted Traffic(Q)')
    plt.legend()
    plt.grid()
    plt.show()

    # df = pd.DataFrame({'value': y_pred, 'real': y_test})
    # df.to_csv('/Users/danawls/Desktop/*Important*/traffic-deep-learning-research/table-figure/table/clock-compare/arima.csv', index=False)
else:
