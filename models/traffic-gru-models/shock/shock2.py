...
# 예를 들어 'Close' 가격의 변화를 충격파로 간주
data['Price_Change'] = data['Close'].diff()  # 가격 변화율 계산
data['Shock_Intensity'] = np.where(data['Price_Change'].abs() > data['Price_Change'].std(), 1, 0)  # 표준편차 이상 변화는 충격파로 간주
...
# 3. 모델 학습
history = model.fit([X_train, shock_train], y_train, epochs=100, batch_size=32, validation_split=0.2)
