...
ann_out = Flatten()(input_ann)  # Flatten the input to make it compatible with Dense layers
ann_out = Dense(64, activation='relu')(ann_out)
ann_out = Dense(32, activation='relu')(ann_out)
shock_intensity_output = Dense(1, activation='linear')(ann_out)  # 각 시퀀스에 대해 하나의 충격파 강도를 예측
...
# 예측된 충격파 강도를 시퀀스 단위로 맞춤 (GRU 모델의 입력에 맞게 변환)
shock_train_pred = shock_train_pred.reshape(-1, 1)
shock_test_pred = shock_test_pred.reshape(-1, 1)
