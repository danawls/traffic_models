...
for i, file_path in enumerate(file_list):
    ...
    if model is None:
        ...
        # 모델 파인튜닝 (이후 파일들)
        shock_train_pred = model.predict([X_train, shock_train])
        shock_test_pred = model.predict([X_test, shock_test])

    # 3. 모델 학습 또는 파인튜닝
    model.fit([X_train, shock_train_pred], y_train, epochs=50, batch_size=32, validation_split=0.2)
