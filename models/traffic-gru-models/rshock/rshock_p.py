...
def create_sequences(data, seq_length=10):
    ...
    for i in range(len(data) - seq_length):
        seq = data.iloc[i:i + seq_length][['traffic(Q)', 'speed(u)', 'confusion', 'lane_number', 'shock_intensity']].values
...
# 모델 초기화
hidden_size = HIDDEN_SIZE  # 은닉 크기 증가
...
# 학습 스케줄러 정의
lr_schedule = tf.keras.callbacks.LearningRateScheduler(lambda epoch: LEARNING_RATE * (0.5 ** (epoch // 10)))
...
for k in range(1):
    ...
    # 데이터 로드 및 전처리
    file_lists = sorted(
        glob.glob(f'/Users/danawls/Desktop/*Important*/traffic-deep-learning-research/test_data/{t}/*.csv'))

    file_list = file_lists[-3:]
    pred_file = f'/Users/danawls/Desktop/*Important*/traffic-deep-learning-research/test_data/{t}/6000VDS03500.csv'
...
preds = gru_model(sequences, training=False)
...
# df.to_csv('/Users/danawls/Desktop/*Important*/traffic-deep-learning-research/table-figure/table/ar/rshock.csv', index=False)
